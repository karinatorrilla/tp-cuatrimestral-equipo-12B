    ...
    public partial class RecuperarPassword : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    /*Verificamos si al llegar a la pagina hay un token */
                ...
                {
                    panelRecuperarPass.Visible = true;
                }
        ...
        protected void btnRecuperar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string correoUsuario = txtUsuario.Text;
                medico = medicosNegocio.buscarCorreoUsuario(correoUsuario);
                if (medico != null && !string.IsNullOrEmpty(medico.Email))
                {
                    string token = Guid.NewGuid().ToString();
                    string linkRecuperacion = $"https://localhost:44318/RecuperarPassword.aspx?token={token}";

                    recuperacion.guardarTokenRecuperacion(medico.Id, token);
